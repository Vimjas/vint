...
class TestScopeDetector(unittest.TestCase):
    def test_detect_scope_visibility(self):
        test_cases = [
        ...
        for (context_scope_visibility, id_node, expected_scope_visibility, expected_implicity) in test_cases:
            scope = create_scope(context_scope_visibility)
            scope_visibility_hint = detect_scope_visibility(id_node, scope)

            expected_scope_visibility_hint = create_scope_visibility_hint(
                expected_scope_visibility,
                is_implicit=expected_implicity
            )
            self.assertEqual(expected_scope_visibility_hint, scope_visibility_hint, id_node)
    ...
    def test_normalize_variable_name(self):
        ...
        for (context_scope_visibility, node, expected_variable_name) in test_cases:
            scope = create_scope(context_scope_visibility)
            normalized_variable_name = normalize_variable_name(node, scope)

            self.assertEqual(expected_variable_name, normalized_variable_name)


    def test_is_builtin_variable(self):
        test_cases = [
            ('my_var', False, False),
            ('count', False, True),
            ('v:count', False, True),
        ...
        for (id_value, is_function, expected_result) in test_cases:
            id_node = create_id(id_value, is_function=is_function)
            result = is_builtin_variable(id_node)

            self.assertEqual(expected_result, result, id_value)


    def test_get_explicity_of_scope_visibility(self):
        test_cases = [
        ...
        for (node, expected_result) in test_cases:
