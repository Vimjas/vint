    ...
    def parse_argopt(self):
        while self.reader.p(0) == "+" and self.reader.p(1) == "+":
            ...
            if viml_eqregh(s, "^++bin\\>"):
                ...
                raise VimLParserException(Err("E474: Invalid Argument", self.reader.pos()))
            else:
                break
            self.reader.skip_white()
...
# FIXME: validate argument
    ...
    neovim_additional_commands = [AttributeDict({"name":"tnoremap", "minlen":8, "flags":"EXTRA|TRLBAR|NOTRLCOM|USECTRLV|CMDWIN", "parser":"parse_cmd_common"})]
...
#        func{ti}on(expr1, ...)
    def parse_expr9(self):
        ...
        if token.type == TOKEN_NUMBER:
            ...
            self.reader.seek_set(pos)
            node = self.parse_identifier()
        elif 0 and (token.type == TOKEN_COLON or token.type == TOKEN_SHARP):
